...
public class ItemStackEditor {
    ...
    public @Nonnull ItemStackEditor removeAllEnchantments() {
        return andMetaConsumer(meta -> meta.getEnchants().keySet().forEach(meta::removeEnchant));
    }

    public @Nonnull ItemStackEditor setUnbreakable(boolean unbreakable) {
        return andMetaConsumer(meta -> meta.setUnbreakable(unbreakable));
    }
