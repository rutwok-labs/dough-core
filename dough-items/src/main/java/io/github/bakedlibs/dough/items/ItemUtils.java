...
public final class ItemUtils {
    ...
    public static @Nonnull ItemStack cloneWithAmount(@Nonnull ItemStack item, int amount) {
        ...
        if (item.getType() != Material.AIR && item.getAmount() > 0) {
            ...
            if (!ignoreEnchantments && item.getEnchantments().containsKey(Registry.ENCHANTMENT.get(NamespacedKey.minecraft("unbreaking")))) {
                ...
                for (int i = 0; i < damage; i++) {
                    if (ThreadLocalRandom.current().nextDouble() * 100 <= (60 + Math.floorDiv(40, (level + 1)))) {
                        remove--;
                    }
                }
            }
