...
public final class ProtectionManager {
    ...
    private void registerModule(Plugin plugin, Function<Plugin, ProtectionModule> constructor) {
        try {
            ...
            if (debug) {
                logger.log(Level.SEVERE, "Failed to load protection module for " + plugin.getName(), x);
            } else {
                logger.log(Level.WARNING, "Failed to load protection module for " + plugin.getName() + " (" + x.getMessage() + ")");
            }
        }
    }
    ...
    public void unregisterModule(@Nonnull Class<? extends ProtectionModule> moduleClass) {
        protectionModules.removeIf(moduleClass::isInstance);
    }

    @ParametersAreNonnullByDefault
    public boolean hasPermissionAll(OfflinePlayer player, Location location, Interaction action) {
        for (ProtectionModule module : protectionModules) {
            try {
                ...
                logger.log(Level.WARNING, "ProtectionModule threw an exception", x);
                return false;
            }
        }
