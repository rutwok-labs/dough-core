...
public class RandomizedSet<T> implements Iterable<T>, Streamable<T> {
    ...
    public void addAll(@Nonnull Collection<T> items, float weight) {
        for (T item : items) {
            add(item, weight);
        }
    }

    public void merge(@Nonnull RandomizedSet<T> other) {
        for (WeightedNode<T> node : other.internalSet) {
            add(node.getObject(), node.getWeight());
        }
    }
    ...
    public Set<T> getRandomSubset(@Nonnull Random random, int size) {
        ...
        java.util.List<WeightedNode<T>> copy = new ArrayList<>(internalSet);
        Collections.shuffle(copy, random);
        return copy.stream().limit(size).map(WeightedNode::getObject).collect(Collectors.toSet());
    }
