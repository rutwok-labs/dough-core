...
public final class TriStateOptional<T> {
    ...
    public void ifComputed(@Nonnull Consumer<T> action) {
        if (state == TriState.COMPUTED) {
            action.accept(value);
        }
    }

    public void ifComputedOrElse(@Nonnull Consumer<T> action, @Nonnull Runnable emptyAction) {
        if (state == TriState.COMPUTED) {
            action.accept(value);
        } else if (state == TriState.EMPTY) {
            emptyAction.run();
        }
    }
