...
public final class BlockPosition {
    ...
    public boolean isChunkLoaded() {
        World ref = this.world.get();
        return ref != null && ref.isChunkLoaded(getChunkX(), getChunkZ());
    }
    ...
    public static boolean isValidY(int y) {
        return y >= -2048 && y <= 2047;
    }

    public double distanceTo(@Nonnull BlockPosition other) {
        return Math.sqrt(distanceSquaredTo(other));
    }

    public double distanceSquaredTo(@Nonnull BlockPosition other) {
        World thisWorld = this.world.get();
        World otherWorld = other.world.get();

        if (thisWorld == null || otherWorld == null || !thisWorld.getUID().equals(otherWorld.getUID())) {
    ...
    public boolean equals(Object o) {
        if (o instanceof BlockPosition) {
            ...
            World thisWorld = this.world.get();
            World otherWorld = pos.world.get();

            if (thisWorld == null || otherWorld == null) {
                return false;
            }

            return thisWorld.getUID().equals(otherWorld.getUID()) && this.position == pos.position;
        }
