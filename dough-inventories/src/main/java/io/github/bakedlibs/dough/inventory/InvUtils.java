...
public final class InvUtils {
    ...
    public static boolean removeItem(@Nonnull Inventory inv, int amount, boolean replaceConsumables, @Nonnull Predicate<ItemStack> predicate) {
        return removeItemAndCount(inv, amount, replaceConsumables, predicate) == amount;
    }

    public static int removeItemAndCount(@Nonnull Inventory inv, int amount, boolean replaceConsumables, @Nonnull Predicate<ItemStack> predicate) {
        int removed = 0;
        for (int slot = 0; slot < inv.getSize(); slot++) {
            ...
            if (item != null && predicate.test(item)) {
                ...
                if (item.getAmount() >= needed) {
                    ItemUtils.consumeItem(item, needed, replaceConsumables);
                    return amount;
                } else {
    ...
    public static int countItem(@Nonnull Inventory inv, @Nonnull Predicate<ItemStack> predicate) {
        int count = 0;
    ...
    public static @Nullable ItemStack addItem(@Nonnull Inventory inv, @Nonnull ItemStack item, int... slots) {
        if (slots.length == 0) {
            Map<Integer, ItemStack> result = inv.addItem(item.clone());
        ...
        for (int slot : slots) {
            ...
            if (current == null || current.getType().isAir()) {
                ...
                return null;
            } else if (ItemUtils.canStack(current, remaining) && isValidStackSize(current, remaining, inv)) {
                int newAmount = current.getAmount() + remaining.getAmount();
                int max = current.getMaxStackSize();
                current.setAmount(Math.min(newAmount, max));

                if (newAmount > max) {
                    remaining.setAmount(newAmount - max);
                } else {
                    remaining = null;
                }
            }
