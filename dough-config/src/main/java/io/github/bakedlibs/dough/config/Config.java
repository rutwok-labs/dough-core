...
public class Config {
    ...
    public void setValue(@Nonnull String path, Object value) {
        if (value == null) {
            ...
            this.store(path, ((OfflinePlayer) value).getUniqueId().toString());
    ...
    public @Nonnull Optional<Chunk> getChunkOptional(@Nonnull String path) {
        return getWorldOptional(path + ".world").map(w -> w.getChunkAt(getInt(path + ".x"), getInt(path + ".z")));
    }
    ...
    public @Nonnull Optional<World> getWorldOptional(@Nonnull String path) {
        String name = getString(path);
        return name != null ? Optional.ofNullable(Bukkit.getWorld(name)) : Optional.empty();
    }
    ...
    public @Nonnull Set<String> getAllKeys() {
        return fileConfig.getKeys(true);
    }
    ...
    public @Nonnull Set<String> getKeys(@Nonnull String path, boolean deep) {
        ConfigurationSection section = fileConfig.getConfigurationSection(path);
        return section == null ? new HashSet<>() : section.getKeys(deep);
    }
    ...
    public void reloadWithDefaults(@Nonnull Plugin plugin) {
